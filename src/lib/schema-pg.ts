/**
 * ClinicMaxx schema, PostgreSQL dialect (Supabase).
 *
 * Differences from the SQLite version in `schema.ts`, and why:
 *
 *  · `INTEGER PRIMARY KEY AUTOINCREMENT` becomes
 *    `GENERATED BY DEFAULT AS IDENTITY`. "BY DEFAULT" rather than "ALWAYS" so the
 *    migration can insert explicit ids and preserve every foreign key; the
 *    sequences get fast-forwarded afterwards.
 *
 *  · Boolean-ish flags stay as `smallint` holding 0/1 rather than becoming real
 *    booleans, so the `= 1` comparisons throughout the query layer keep working
 *    unchanged. Not the prettiest Postgres, but it keeps the port honest.
 *
 *  · `appointments.invoice_id` references `invoices`, which is created later.
 *    SQLite resolves that lazily; Postgres does not, so the constraint is added
 *    with ALTER TABLE once both tables exist.
 *
 *  · Row Level Security is enabled on every table with NO policies. Supabase
 *    publishes everything in the `public` schema through PostgREST, so without
 *    this, anyone holding the anon key could read the entire clinical record.
 *    Deny-by-default costs us nothing: the app connects as the table owner,
 *    which bypasses RLS.
 */
...
/** Every table, in dependency order — used by the migration and loader. */
export const TABLES = [
  "clinic",
  "locations",
  "disciplines",
...
/**
 * Deny-by-default. Supabase exposes the whole `public` schema over PostgREST, so
 * every table needs RLS on even though this app never uses the anon key — one
 * leaked key should not equal one leaked clinic. The app connects as the table
 * owner, which is exempt, so nothing else has to change.
 */
export const ENABLE_RLS = TABLES.map(
  (t) => `ALTER TABLE ${t} ENABLE ROW LEVEL SECURITY;`,
).join("\n");

/** Fast-forward identity sequences after a bulk load with explicit ids. */
export const RESET_SEQUENCES = TABLES.filter(
  (t) => t !== "clinic" && t !== "practitioner_treatments",
)
  .map(
    (t) =>
